<template>
  <div>
    ...
    <div v-if="false">
      <div class="Record" v-if="duo1">
        ...
        <div class="Record_content_box">
          <div
            :class="{legal_content:changeClass,legal2_content:changeStyle}"
            v-for="(item,index) in jiben2"
            :key="index"
          >
            <h1>基本信息</h1>
            <ul v-if="tiqu">
              <li v-for="(items,index) in item.contentList" :key="index">
                <b>{{items.key}}:</b>
                {{items.value}}
              </li>
            </ul>
            <div class="checkbox" v-if="changeStyle">
              <input type="checkbox" v-model="box" :value="index" />
            </div>
          </div>
          <div class="kong"></div>
      ...
      <div class="footer" v-if="duo1">
        ...
        <div class="footer_btn2" @click="QRcode">
    ...
    <div v-if="true">
      ...
      <!-- <div class="QR" v-if="QR" @click="QRshow"> -->
      <div class="QR" v-if="false" @click="QRshow">
        <div class="QR_wrap">
          <div class="QR_header">
            <div class="QR_text">请向窗口操作人员出示</div>
          </div>
...
export default {
  ...
  methods: {
    ...
    getUrl(e) {
      ...
      console.log(e.url + "?code=" + this.code)
      axios
        .get(e.url + "?code=" + this.code)
        .then(res => {
          console.log(JSON.parse(res.data.data))
          console.log(JSON.parse(res.data.data)[0].contentList.length);
          res.data.data[2] == "]" || res.data.data == ""
            ? (this.localStorage = false)
            : (this.localStorage = true);
          if (JSON.parse(res.data.data)[0].contentList.length>0) {
            window.localStorage.setItem("data", JSON.stringify(JSON.parse(res.data.data)[0].contentList));
  ...
  created() {
    ...
    // 个人
    // axios.get('http://120.55.60.206:8081/api/userServiceList?code=' + this.code)
    axios
      .get("http://120.221.95.112:18080/api/enterpriseServiceList?code=" + this.code)
      .then(res => {
        this.title = JSON.parse(res.data.data).sort(function(a, b) {
